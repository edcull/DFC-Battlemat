...
- **Combat (in progress):** being reworked into a *server-driven* multi-step resolution — each modal decision becomes an intent, the server rolls every die with the room's seeded rng, and the defender makes their own defensive choices. Steps 1–2 done: the post-save resolution core (`resolveAttackDamage` / `proceedQueues` / `rollDeferredBackupSaves`) **and** the to-hit / primary-save dice-rolling (`rollHits` / `rollSaves`, with the coherency helpers `coherencyInches` / `outOfFormationSet` / `groupInFormation`) are extracted into `mutators.js` — shared, shimmed in the client, behaviour unchanged; the rolls are no longer render side-effects. Remaining: convert the modal buttons to decision intents, route defender choices, and render the modal from broadcast state.
