...
- **Scenario Expansion 1 tab:** behaves like the Standard tab — a dropdown of the 14 Scenario Expansion 1 scenarios (Ready Salted Earth, Erupting Quarters, Latitudinal Lanes, Lagrange Points, When Backfields Meet, Very Important Moon, Moonshot, Moonwreck, Moonbreaker, Moonguard, Moonswipe, Moonskipper, One With (Almost) Nothing, (Almost) Nothing At All) plus a RANDOM button. Selecting/rolling one applies a full preset from `EXPANSION_PRESETS` and shows a read-only summary of Deployment Type, Approach, Scoring Objective, a Scenery & Dropsites row, and a **Special Rules** briefing (`SE1_NOTES`) listing the bespoke effects that are adjudicated manually. Each scenario's layout is a bespoke `LAYOUTS` entry (`se1_*`, transcribed from the official maps). The expansion adds three new `APPROACH_BEHAVIOURS` (Imminent, Backline, Staggered) wired into `canActivateOffTable`, the matching bespoke `APPROACHES` combos (`all_imminent`/`all_backline`/`all_staggered` and the asymmetric `exp_attacker_staggered`/`exp_defender_imminent`), and five bespoke `OBJECTIVES` (Normal, Demolish, Focal Points, Kill Points, Assess). Kill Points is engine-scored (game-end +2 VP/500 pts destroyed, like Attrition); Normal reuses control/contest scoring.
...
- **Scenario Expansion 1** scenarios apply faithful dropsite layouts, deployment/approach timing (Imminent/Backline/Staggered round-gating) and a scoring objective, and Kill Points / Normal scoring are engine-scored. The remaining bespoke effects are adjudicated manually and surfaced as Special Rules notes in the scenario summary: Focal-Point and board-quarter value scoring, the Assess action, Demolish immediate-on-Level awards, asymmetric colour-city ownership (When Backfields Meet), moving/breaking moons (Moonskipper, Moonbreaker, Moonswipe repositioning), Orbit-only Orbital Decay damage (the "(Almost) Nothing" pair), and per-scenario end-game bonuses. The Staggered approach's "X Groups per round" count is also tracked manually (any off-table Group may arrive).
