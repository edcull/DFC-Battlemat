...
- **Combat (in progress):** being reworked into a *server-driven* multi-step resolution — each modal decision becomes an intent, the server rolls every die with the room's seeded rng, and the defender makes their own defensive choices. Step 1 done: the pure post-save resolution core (`resolveAttackDamage` / `proceedQueues` / `rollDeferredBackupSaves`) is extracted into `mutators.js` (shared, shimmed in the client, behaviour unchanged). Remaining: lift hit/save dice-rolling out of `renderAttackModal` into engine functions, convert the modal buttons to decision intents, route defender choices, and render the modal from broadcast state.
